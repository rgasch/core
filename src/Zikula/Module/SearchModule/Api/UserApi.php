...
class UserApi extends \Zikula\Framework\Api\AbstractApi
{
    ...
    public function search($args)
    {
        ...
        if ($firstPage) {
            ...
            $query = $this->entityManager->createQuery("DELETE FROM SearchModule\Entity\SearchResultEntity s WHERE s.sessid = '$sessionId' OR OR DATE_ADD(s.found, INTERVAL 8 HOUR) < NOW()");
            $query->execute();
                ...
                if (isset($mod['functions']) && (empty($active) || isset($active[$mod['title']]))) {
                    foreach ($mod['functions'] as $contenttype => $function) {
                        ...
                        if (!$ok) {
                            ...
                            return $this->redirect(ModUtil::url('Search', 'user', 'index'));
                        }
                    }
                }
            }
        ...
        $checker = new \SearchModule\Helper\ResultHelper($searchModulesByName);

        $dql = "SELECT FROM SearchModule\Entity\SearchResultEntity s WHERE s.sessid = '$sessionId' s.sort";
    ...
    public function getall($args)
    {
        ...
        $dql = "SELECT FROM SearchModule\Entity\SearchStatEntity s $sort";
        $query = $this->entityManager->createQuery($dql);
    ...
    public function log($args)
    {
        ...
        if (!$obj) {
            $obj = new \SearchModule\Entity\SearchStatEntity();
    ...
    public function decodeurl($args)
    {
        ...
        $action = $this->request->attributes->get('_action');
        if ($action == 'recent' && isset($args['vars'][$nextvar])) {
            System::queryStringSetVar('startnum', $args['vars'][$nextvar]);
        }

        // identify the correct parameter to identify the page
        if ($action == 'search' && isset($args['vars'][$nextvar]) && !empty($args['vars'][$nextvar])) {
