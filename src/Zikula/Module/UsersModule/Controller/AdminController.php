...
class AdminController extends \Zikula\Framework\Controller\AbstractController
{
    ...
    public function displayRegistrationAction()
    {
        ...
        if (empty($uid) || !is_numeric($uid)) {
            $this->registerError(LogUtil::getErrorMsgArgs());
            return $this->redirect(ModUtil::url($this->name, 'admin', 'viewRegistrations', array('return' => true)));
        }
        ...
        return $this->response($this->view->assign('reginfo', $reginfo)
            ->assign('actions', $actions)
    ...
    public function modifyRegistrationAction()
    {
        ...
        $formData = new Users_Controller_FormData_ModifyRegistrationForm('users_modifyreg', $this->container);
        $errorFields = array();
        $errorMessages = array();
        ...
        if ($this->request->getMethod() == 'POST') {
            ...
            $this->dispatcher->dispatch('module.users.ui.validate_edit.modify_registration', $event);
            $validators = $event->getData();
            ...
            if (!$errorFields && !$validators->hasErrors()) {
                ...
                if ($originalRegistration['uname'] != $registration['uname']) {
                    ...
                    $this->dispatcher->dispatch('user.registration.update', $updateEvent);
                }
    ...
    public function configAction()
    {
        ...
        if ($this->request->getMethod() == 'POST') {
            ...
            if ($configData->isValid()) {
                ...
                $this->dispatcher->dispatch('module.users.config.updated', $event);
            } else {
    ...
    public function exporterAction(array $args = array())
    {
        ...
        return $this->response($this->view->fetch('Admin/export.tpl'));
    }
    ...
    public function toggleForcedPasswordChangeAction()
    {
        if ($this->request->getMethod() == 'GET') {
            ...
            return $this->response($this->view->assign('user_obj', $userObj)
